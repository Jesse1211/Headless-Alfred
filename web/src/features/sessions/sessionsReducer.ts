...
export function reducePerSession(
  ...
  switch (m.type) {
    ...
    case 'chunk': {
      ...
      // wrong RIGHT NOW. We use a simple "have we seen this exact
      // payload as the tail of the buffer just now?" check.
      const data = b64decode(m.data)
      if (data && cur.running.output.endsWith(data) && data.length > 0) {
        // Likely a duplicate of the most recent chunk — skip.
        // This is heuristic; the only false positive is a command
        // that legitimately produces the same bytes twice in a row
        // RIGHT at the boundary (e.g. `printf 'aa'`), which is
        // benign in practice.
        return { perSession: prev }
      }
      const next = new Map(prev)
