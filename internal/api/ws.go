...
func runClientLoop(conn *websocket.Conn, m *session.Manager, bridge *claude.Bridge, disp *claude.Dispatcher, runner *claude.Runner, broadcaster *recapBroadcaster, disk *diskBroadcaster, csMgr *claudestate.SessionManager) {
	...
	sessions := m.ListAll()
	c := newWSConn(m, disp, broadcaster, disk, sessions)
	defer c.close()
	...
	defer func() {
		...
		for _, e := range c.claudeRunStates.takeAll() {
			stopRun(e.state)
	...
	for {
		...
		case msg, ok := <-inbound:
			...
			handleInbound(msg, m, bridge, runner, c.claudeEvents, c.claudeRunStates, csMgr, write, c.connCtx, c.bgLogSubs, c.bgMeta, c.bgCWD)
		case ev, ok := <-c.events:
			if !ok {
				return
			}
		...
		case sid := <-c.closedCh:
			...
			if st, ok := c.claudeRunStates.take(sid); ok {
				stopRun(st)
		...
		case sid := <-c.createdCh:
			...
			go forwardSubscriber(sid, sub, c.events, c.stop)
			rawSub := sh.SubscribeRaw(64)
			c.cancels = append(c.cancels, rawSub.Close)
			go forwardRaw(sid, rawSub, c.ptyChunks, c.stop)
			if disp != nil {
